import type { Messages } from "../types";

const en: Messages = {
  meta: {
    title: "MeTi Reformer Studio | Clinical Pilates in Limassol",
    description:
      "Physiotherapist and Clinical Pilates instructor. Reformer sessions in small groups — personalized care, safe movement, lasting results. Book online.",
  },
  nav: {
    sessions: "Sessions",
    signIn: "Sign in",
    signOut: "Sign out",
    account: "Account",
    bookNow: "Book now",
    menu: "Menu",
    closeMenu: "Close menu",
  },
  hero: {
    eyebrow: "Clinical Pilates & Reformer",
    title: "Your health, your movement.",
    description:
      "Physiotherapy-based reformer pilates in small groups — personalized care, safe movement, and lasting results.",
    bookSession: "Book reformer",
    viewSessions: "View details",
    imageAlt: "Reformer pilates — book a session in Limassol",
    priceLine: "€{price} · 45 min",
    payAtStudio: "Pay at the studio",
    classSize: "Up to {count} people",
  },
  visit: {
    title: "Visit the studio",
    subtitle: "Agios Ioannis Pitsilias, Limassol. Book online or call.",
    hours: "Hours",
    price: "Price",
    priceDetail: "€{price} · 45 minutes · pay at the studio",
    classSize: "Class size",
    classSizeDetail: "Small groups, up to {count} people",
    address: "Address",
    directions: "Open in Google Maps",
    phone: "Phone",
    email: "Email",
    book: "Book a time",
    policyNote:
      "Cancel at least {hours} hours before or the session is still payable. Classes start on time and do not wait for late arrivals.",
  },
  how: {
    title: "How to book",
    subtitle: "Reserve a reformer place on your phone in under a minute.",
    steps: [
      {
        title: "Pick a day",
        description: "Classes are Tuesday, Thursday, and Saturday. The next open day is highlighted.",
      },
      {
        title: "Choose a time",
        description: "Each slot has up to {count} places. Full times stay visible so you can pick another.",
      },
      {
        title: "Confirm",
        description:
          "Leave your name and email. Payment is at the studio. Cancel at least {hours} hours before, or the session is still payable. Classes start on time.",
      },
    ],
  },
  about: {
    title: "About us | Meropi Tirri",
    intro:
      "Meropi Tirri is a specialized physiotherapist, graduate of the Physiotherapy Department of the International Hellenic University (formerly TEI Thessaloniki). Since 2015 she has practiced physiotherapy with absolute dedication, continually investing in broadening her knowledge through specialized seminars and high-level postgraduate programs.",
    certificationsIntro:
      "With the primary goal of holistic treatment and optimal recovery for every patient, she has earned certifications across a wide range of modern therapeutic methods:",
    certifications: [
      {
        name: "OMT Manual Therapist",
        detail: "Specialized musculoskeletal physiotherapy",
      },
      {
        name: "Biomedical Acupuncture",
        detail: "Specialized since 2020",
      },
      {
        name: "Reflexology & Aromatherapy",
        detail: "",
      },
      {
        name: "Therapeutic Cupping",
        detail: "",
      },
      {
        name: "Hippotherapy",
        detail: "",
      },
      {
        name: "Neuromuscular Massage",
        detail: "",
      },
    ],
    specialization:
      "In recent years, focusing on therapeutic exercise and prevention, she specializes as a Clinical Pilates & Reformer Pilates Instructor.",
    philosophyTitle: "Our center's philosophy",
    philosophyParagraph1:
      "In our modern, privately owned space, we design together with you a personalized treatment and training plan that responds precisely to your own needs.",
    philosophyParagraph2:
      "By adopting the philosophy of small groups, we ensure that every participant enjoys the personal attention and strict supervision they deserve. The flow of each class is adapted to guarantee the correct and safe execution of every movement.",
    programIntro:
      "Based entirely on the principles of physiotherapy, our program is ideal for:",
    programBenefits: [
      "Improving body posture",
      "Effectively strengthening the core",
      "Immediate relief from chronic musculoskeletal pain",
    ],
    closingTitle: "Invest in your health and well-being.",
    closingText:
      "As places in our groups are limited, book your appointment early through our site and feel the difference from the very first sessions.",
  },
  sessions: {
    label: "Reformer",
    title: "Book a reformer session",
    subtitle: "45 minutes on the carriage. Live availability.",
    mostPopular: "Reformer",
    book: "Book",
    fromPrice: "from",
    readyTitle: "Ready to move?",
    readySubtitle: "Pick a time and reserve your reformer spot in under a minute.",
    types: {
      reformer: {
        name: "Reformer",
        description: "Spring-loaded carriage work for strength, length, and control.",
        duration: "45 min",
      },
    },
  },
  footer: {
    book: "Book",
    contact: "Contact",
    privacy: "Privacy",
    terms: "Terms",
    faq: "FAQ",
    cookies: "Cookies",
    refunds: "Cancellations",
    licenses: "Licenses",
  },
  cookieNotice: {
    message: "We use only essential cookies to run bookings and remember your language.",
    link: "Cookie policy",
    dismiss: "OK",
  },
  book: {
    back: "Back",
    studioUnavailable: "Studio not available",
    scheduleError: "Could not load schedule.",
    loadError: "Something went wrong. Please refresh the page.",
  },
  booking: {
    stepOf: "Step {current} of {total}",
    steps: {
      service: "Session",
      date: "Date",
      time: "Time",
      summary: "Confirm",
    },
    selectSession: "Select a session",
    selectSessionSub: "Choose what you'd like to book",
    pickDate: "Pick a date",
    pickDateSub: "Classes are Tuesday, Thursday, and Saturday. Highlighted days still have open times.",
    nextOpenDay: "Next open day is {date}.",
    pickTime: "Pick a time",
    noSlots: "No times left this day. Classes may already have finished.",
    tryAnotherDay: "Go back and tap Saturday, Tuesday, or Thursday.",
    spotsLeft: "{remaining} left",
    slotFull: "Full",
    confirm: "Confirm",
    confirmSub: "Review your booking details",
    session: "Session",
    sessionPrice: "Session price",
    date: "Date",
    time: "Time",
    total: "Total",
    reschedulePolicy: "Free cancel or change up to {hours} hours before your session.",
    lateCancelPolicy: "If you do not cancel in time, the session must still be paid at the studio.",
    punctualityPolicy: "Classes start on time and do not wait if you arrive late.",
    continuePayment: "Continue to payment",
    confirmBooking: "Confirm booking",
    processing: "Processing…",
    serviceNames: {
      "Reformer Session": "Reformer Session",
    },
  },
  auth: {
    welcomeBack: "Welcome back",
    signInSubtitle: "Sign in to complete your booking",
    createAccount: "Create your account",
    createAccountSubtitle: "Join MeTi Pilates to book reformer sessions",
    continueGoogle: "Continue with Google",
    connecting: "Connecting…",
    orDivider: "or",
    email: "Email",
    password: "Password",
    fullName: "Full name",
    passwordMin: "Password (minimum 8 characters)",
    signingIn: "Signing in…",
    signIn: "Sign in",
    creatingAccount: "Creating account…",
    createAccountBtn: "Create account",
    googleError: "Failed to sign in with Google. Please try again.",
    googleSignUpError: "Failed to sign up with Google. Please try again.",
    emailError: "Incorrect email or password.",
    signInError: "Failed to sign in. Please try again.",
    signUpError: "Failed to create account. Please try again.",
    signUpUnavailable:
      "Account registration is not available on this preview. Book as a guest with your email — no account needed.",
    signUpInvalidOrigin:
      "Sign-up could not verify this site address. Refresh the page and try again.",
    bookWithoutAccount: "Book without an account",
    noAccount: "Don't have an account?",
    signUpFree: "Sign up for free",
    hasAccount: "Already have an account?",
    backHome: "← Back to home",
    forgotPassword: "Forgot password?",
    forgotPasswordTitle: "Reset your password",
    forgotPasswordSubtitle: "Enter your email and we'll send you a reset link.",
    forgotPasswordSent: "Check your email",
    forgotPasswordSentDetail:
      "If an account exists for that email, we sent a password reset link. It expires in 1 hour.",
    forgotPasswordError: "Could not send reset email. Please try again.",
    sendResetLink: "Send reset link",
    sendingResetLink: "Sending…",
    backToSignIn: "Back to sign in",
    resetPasswordTitle: "Choose a new password",
    resetPasswordSubtitle: "Enter a new password for your account.",
    resetPasswordBtn: "Update password",
    resettingPassword: "Updating…",
    resetPasswordSuccess: "Password updated",
    resetPasswordSuccessDetail: "Your password was changed. Redirecting to sign in…",
    resetPasswordError: "Could not reset password. Please request a new link.",
    resetPasswordInvalidLink: "This reset link is invalid or has expired.",
    resetPasswordMismatch: "Passwords do not match.",
    acceptLegalRequired: "Please confirm that you have read the privacy policy and terms.",
    requestNewResetLink: "Request a new reset link",
    newPassword: "New password (minimum 8 characters)",
    confirmPassword: "Confirm new password",
  },
  checkout: {
    title: "Confirm booking",
    titlePayment: "Checkout",
    signInToContinue: "Sign in to continue",
    signInToContinueSub: "Already have an account? Sign in to pre-fill your details.",
    guestContactTitle: "Your details",
    guestContactSub: "Email is required for the confirmation. Name and phone are optional.",
    guestEmailRequired: "Please enter your email to continue.",
    guestEmailInvalid: "Please enter a valid email address.",
    optionalName: "Name (optional)",
    optionalPhone: "Phone (optional)",
    optionalPhoneHint: "If we need to reach you about the session.",
    guestPhoneInvalid: "Enter a valid phone number.",
    continueAsGuest: "Continue as guest",
    testModeTitle: "Test mode",
    testModeSub: "Payment is not real. The studio is testing before going live.",
    paymentMethod: "Payment method",
    paymentUnavailable: "Payment unavailable",
    paymentUnavailableSub: "Online payment is not set up yet. Please contact the studio.",
    mercadoPago: "Mercado Pago",
    mercadoPagoSub: "Credit card, debit card, or account balance",
    securePayment: "Secure payment",
    cancellationPolicy: "Cancellation policy",
    cancelReschedule: "Free cancel or change with {hours} hours notice",
    cancelNoRefund: "Cancel later than {hours} hours before — session still payable",
    cancelNoShow: "No-show — session still payable",
    cancelPunctuality: "Classes start on time and do not wait if you arrive late",
    summary: "Summary",
    total: "Total",
    includesCosts: "Includes all fees",
    signInToPay: "Sign in to pay",
    pay: "Pay {amount}",
    confirmBooking: "Confirm booking",
    bookingOnlyTitle: "Booking only",
    bookingOnlySub:
      "No online payment. Your session is reserved immediately — payment is handled at the studio.",
    bookingPolicy: "Booking policy",
    bookingPolicyReschedule: "Free cancel or change with {hours} hours notice",
    bookingPolicyLatePay:
      "If you do not cancel at least {hours} hours before, the session must still be paid at the studio.",
    bookingPolicyPunctuality: "Classes start on time and do not wait if you arrive late.",
    bookingPolicyContact:
      "Cancel from the link in your confirmation email or from your account, at least {hours} hours before the session.",
    payAtStudio: "Payment at the studio",
    acceptLegalPrefix: "I have read and accept the",
    acceptPrivacy: "privacy policy",
    acceptLegalMiddle: "and the",
    acceptTerms: "terms of service",
    acceptLegalSuffix: ".",
    acceptLegalRequired: "Please confirm that you have read the privacy policy and terms.",
    withdrawalNote:
      "A class on a set date is a leisure service. The 14-day online cooling-off right does not apply. You may cancel free of charge up to {hours} hours before.",
    noBookingData: "No booking data",
    bookSession: "Book a session",
    errorCreateAppointment: "Failed to create booking. Please try again.",
    min: "min",
  },
  bookingManage: {
    title: "Your booking",
    subtitle: "View or cancel this reformer session without signing in.",
    loading: "Loading booking…",
    invalid: "This booking link is invalid or has expired.",
    loadError: "Could not load this booking. Please try again.",
    session: "Session",
    instructor: "Instructor",
    when: "Date and time",
    price: "Price",
    status: "Status",
    cancel: "Cancel booking",
    cancelling: "Cancelling…",
    cancelled: "Your booking has been cancelled and the slot is free again.",
    cannotCancel:
      "This booking can no longer be cancelled online. The session must still be paid at the studio.",
    confirmBody: "Cancel this session? The reformer place will be released.",
    backHome: "← Back to home",
  },
  checkoutResult: {
    checkEmailToManage:
      "We sent a confirmation link to your email. Open it to view or cancel this booking.",
    manageThisBooking: "View or cancel this booking",
    signInToView:
      "Sign in to view your booking, or open the confirmation link we emailed you.",
    noBookingInfo: "No booking information available.",
    bookingConfirmed: "Booking confirmed!",
    bookingConfirmedSub:
      "Your reformer session is booked. Arrive a few minutes early — classes start on time and do not wait. Cancel at least {hours} hours before, or the session is still payable.",
    paymentConfirmed: "Booking confirmed!",
    paymentConfirmedSub:
      "Your reformer session is booked. Arrive a few minutes early — classes start on time and do not wait. Cancel at least {hours} hours before, or the session is still payable.",
    goToDashboard: "Go to my bookings",
    paymentFailed: "Payment was not completed",
    paymentFailedSub: "No charge was made. You can try again whenever you like.",
    bookAgain: "Book again",
    confirmingPayment: "Confirming your payment…",
    confirmingPaymentSub: "This usually takes a few seconds.",
    stillPending: "Still pending. Check your booking in a few minutes.",
    refreshStatus: "Refresh status",
    toastConfirmedTitle: "Booking confirmed!",
    toastConfirmedSub: "Your reformer session is booked.",
  },
  dashboard: {
    dashboard: "Dashboard",
    bookSession: "Book session",
    myAppointments: "My bookings",
    myReviews: "My reviews",
    myProfile: "My profile",
    client: "Client",
    signOut: "Sign out",
    loading: "Loading…",
    hello: "Hello, {name}!",
    welcome: "Welcome to your bookings",
    bookSessionTitle: "Book a session",
    bookSessionSub: "Tue, Thu & Sat · next open times are highlighted",
    myBookingsTitle: "My bookings",
    myBookingsSub: "View sessions, payment status, or cancel if plans change",
    account: "Account",
    nameLabel: "Name",
    emailLabel: "Email",
    phoneLabel: "Phone",
    bookingsPageTitle: "My bookings",
    bookingsPageSub: "See your sessions, whether a class is paid, and cancel if your plans change",
    filterAll: "All",
    filterUpcoming: "Upcoming",
    filterPast: "Past",
    noBookings: "No bookings yet",
    noBookingsSub: "Book your first reformer session online.",
    statusConfirmed: "Confirmed",
    statusInProgress: "In progress",
    statusCompleted: "Completed",
    statusCancelled: "Cancelled",
    statusPending: "Payment pending",
    statusNoShow: "No-show",
    sessionPaid: "Paid",
    sessionUnpaid: "Unpaid",
    withInstructor: "with {name}",
    sessionWith: "{service} with {name}",
    minutes: "{count} min",
    pay: "Pay",
    cancel: "Cancel",
    tooLateToCancel: "Too late to cancel — session still payable",
    review: "Review",
    close: "Close",
    saving: "Saving…",
    yourReview: "Your review",
    rateSession: "Rate your session",
    reviewPlaceholder: "Tell us about your experience (optional)…",
    updateReview: "Update review",
    submitReview: "Submit review",
    reviewThanks: "Thank you for your review!",
    reviewThanksSub: "Your rating helps other clients.",
    reviewError: "Could not save review.",
    cancelBookingTitle: "Cancel booking",
    cancelConfirmedBody:
      "Cancel your {service} on {when}? The slot will be released for other clients.",
    cancelUnpaidBody: "Cancel your unpaid booking for {service}? The time slot will be released.",
    bookingCancelled: "Booking cancelled",
    bookingCancelledSub: "Your session was cancelled and the slot is available again.",
    cancelError: "Could not cancel appointment.",
    cancelReasonClient: "Cancelled by client",
    profileTitle: "My profile",
    profileSub: "Manage your personal information",
    personalInfo: "Personal information",
    personalInfoSub: "Update your name, phone, and profile photo",
    profilePhoto: "Profile photo",
    photoHint: "JPEG, PNG or WebP (max. 2MB)",
    removePhoto: "Remove photo",
    yourName: "Your name",
    phoneOptional: "Phone (optional)",
    saveChanges: "Save changes",
    profileUpdated: "Profile updated successfully",
    profileUpdateError: "Failed to update profile",
    connectionError: "Connection error",
    uploadError: "Failed to upload image",
    privacyTitle: "Your data",
    privacySub: "Download a copy or delete your account.",
    downloadData: "Download my data",
    downloadingData: "Preparing…",
    downloadError: "Could not download your data. Please try again.",
    deleteAccount: "Delete my account",
    deleteAccountSub:
      "This removes your name, email, phone, and photo. Cancel any upcoming session first.",
    deleteAccountConfirm:
      "Delete your account? This cannot be undone. Upcoming bookings must be cancelled first.",
    deleteAccountBlocked: "Cancel your upcoming sessions first, then try again.",
    deletingAccount: "Deleting…",
    deleteError: "Could not delete the account. Please try again or email the studio.",
    deleted: "Your account has been deleted.",
  },
  admin: {
    roleAdmin: "Admin",
    roleClient: "Client",
    roleInstructor: "Instructor",
    studioAdmin: "Studio admin",
    signOut: "Sign out",
    navStudio: "Studio",
    navCalendar: "Calendar",
    navSite: "Site",
    navOverview: "Overview",
    navBookings: "Bookings",
    navClients: "Clients",
    navHours: "Hours",
    navClosures: "Closures",
    navWebsite: "Website",
    overviewTitle: "Studio overview",
    overviewSub: "This week’s schedule and today’s sessions",
    bookForClientTitle: "Book a session",
    bookForClientSub: "Open the public calendar to book for a client",
    shortcutBookings: "Bookings",
    shortcutBookingsSub: "Who is coming and free a slot",
    shortcutClients: "Clients",
    shortcutClientsSub: "Names, phone, and session dates",
    shortcutHours: "Hours",
    shortcutHoursSub: "Weekly open days and times",
    shortcutClosures: "Closures",
    shortcutClosuresSub: "Holidays and extra days off",
    thisWeek: "This week",
    openBookings: "Open bookings",
    editHours: "Edit hours",
    sessionsToday: "Sessions today",
    completedToday: "Completed today",
    upcoming: "Upcoming",
    clients: "Clients",
    bookingsTitle: "Bookings",
    bookingsSub: "Upcoming sessions, who is coming, and free a slot if someone cannot attend",
    upcomingBookings: "Upcoming bookings",
    holdingPlaces: "{count} holding a reformer place",
    holdingPlacesOne: "1 holding a reformer place",
    loading: "Loading…",
    bookForClient: "Book for a client",
    freeAllUpcoming: "Free all upcoming",
    freeing: "Freeing…",
    cancelHint: "Cancel a session to release that reformer slot on the public calendar.",
    noUpcoming: "No upcoming bookings holding slots.",
    cancelFreeSlot: "Cancel & free slot",
    cancelling: "Cancelling…",
    testBooking: "test",
    cancelBookingTitle: "Cancel booking",
    cancelBookingBody: "Free the {when} slot for {name}?",
    cancelReasonAdmin: "Cancelled by admin",
    couldNotCancel: "Could not cancel booking",
    freeAllTitle: "Free all upcoming slots",
    freeAllBodyOne: "Cancel the 1 upcoming booking and make that slot available again?",
    freeAllBodyMany: "Cancel {count} upcoming bookings and make those slots available again?",
    couldNotFree: "Could not free slots",
    slotsFreed: "Slots freed",
    slotsFreedBody: "{count} bookings cancelled.",
    slotsFreedBodyOne: "1 booking cancelled.",
    weekSchedule: "Week schedule",
    weekScheduleSub: "Open slots and bookings (Cyprus time)",
    previousWeek: "Previous week",
    nextWeek: "Next week",
    today: "Today",
    noUpcomingDays: "No upcoming days in this week.",
    holiday: "Holiday",
    closed: "Closed",
    holidayClosed: "{name} — closed",
    studioClosed: "Studio closed",
    noSlotsThisDay: "No slots this day",
    open: "Open",
    freeSlot: "Free",
    clientsTitle: "Clients",
    clientsSub: "Open a client for upcoming and completed classes. Mark completed sessions as paid and record who paid.",
    clientsLoadError: "Could not load clients. Try again.",
    clientsRetry: "Try again",
    people: "People",
    upcomingSessions: "Upcoming sessions",
    admins: "Admins",
    searchPeople: "Search name, email, or phone…",
    filterAll: "All",
    filterClients: "Clients",
    filterAdmins: "Admins",
    filterAllBookings: "All bookings",
    filterHasUpcoming: "Has upcoming",
    filterNoUpcoming: "No upcoming",
    filterUnpaid: "Unpaid sessions",
    unpaidTotal: "Unpaid",
    unpaidAmount: "{amount} unpaid",
    personCount: "{count} person",
    peopleCount: "{count} people",
    noClientsYet: "No clients yet",
    noClientsYetSub: "When someone signs up or books a session, they will appear here with their dates.",
    noMatchingPeople: "No matching people",
    noMatchingPeopleSub: "Try a different search or filter. Totals above stay the same.",
    unnamedClient: "Unnamed client",
    joined: "Joined {date}",
    nextSession: "Next session",
    lastSession: "Last session",
    noSessionsYet: "No sessions yet",
    upcomingCount: "Upcoming ({count})",
    recentHistory: "Recent history",
    yearClasses: "This year",
    yearClassCount: "{count} this year",
    yearClassCountOne: "1 this year",
    upcomingSection: "Upcoming",
    completedSection: "Completed",
    yearDates: "Dates this year",
    latestCompleted: "Latest classes",
    noCompletedYet: "No completed classes yet",
    noUpcomingSessions: "No upcoming sessions",
    minutes: "{count} min",
    statusConfirmed: "Confirmed",
    statusPending: "Pending",
    statusInProgress: "In session",
    statusCompleted: "Done",
    statusCancelled: "Cancelled",
    statusNoShow: "No-show",
    sessionPaid: "Paid",
    sessionUnpaid: "Unpaid",
    markPaid: "Mark paid",
    markUnpaid: "Undo payment",
    paidByPlaceholder: "Who paid",
    paidBy: "Paid by {name}",
    paidByRequired: "Enter who paid",
    paymentSaved: "Payment recorded",
    paymentCleared: "Payment cleared",
    paymentError: "Could not update payment. Try again.",
    savingPayment: "Saving…",
    hoursTitle: "Hours",
    hoursSub: "Set the weekly days and times clients can book at {name}. Block holidays or extra days off under",
    closuresLink: "Closures",
    openDays: "Open days",
    daysOpen: "{count} days open for booking",
    daysOpenOne: "1 day open for booking",
    sessionsPerDay: "Sessions per day",
    timeSlots: "{count} time slots",
    sessionGap: "{duration} min · {gap} min gap",
    capacity: "Capacity",
    perSlot: "{count} per slot",
    instructor: "Instructor: {name}",
    hoursHint:
      "Toggle the days you want open each week (e.g. Monday, Wednesday, Saturday). Set start and end times per day. Changes apply immediately on the public booking calendar after you save.",
    cancelHoursLabel: "Free cancellation window",
    cancelHoursUnit: "hours before the session",
    cancelHoursHint:
      "Clients can cancel without charge until this many hours before class. Saving updates booking, emails, checkout, and legal pages.",
    slotCapacityLabel: "Places per class",
    slotCapacityUnit: "people",
    slotCapacityHint:
      "How many clients can book the same time. Saving updates the public calendar and homepage.",
    bookingWeeksLabel: "Booking window",
    bookingWeeksUnit: "weeks ahead",
    bookingWeeksHint:
      "How far in advance clients can book. Saving updates the public calendar.",
    weeklyHours: "Weekly hours",
    saveSchedule: "Save schedule",
    saving: "Saving…",
    start: "Start",
    end: "End",
    gapMin: "Gap (min)",
    slotsCount: "{count} slots",
    lunchStart: "Lunch start (optional)",
    lunchEnd: "Lunch end (optional)",
    closedNotBookable: "Closed — not bookable",
    loadingCalendar: "Loading studio calendar",
    noStudioConfigured: "No studio instructor configured. Run pnpm demo:setup.",
    loadCalendarError: "Could not load studio calendar",
    saveScheduleError: "Could not save schedule",
    saved: "Saved",
    calendarUpdated: "Studio calendar updated",
    connectionError: "Connection error",
    toggleDay: "Toggle {day}",
    weekdaySunday: "Sunday",
    weekdayMonday: "Monday",
    weekdayTuesday: "Tuesday",
    weekdayWednesday: "Wednesday",
    weekdayThursday: "Thursday",
    weekdayFriday: "Friday",
    weekdaySaturday: "Saturday",
    closuresTitle: "Closures",
    closuresSub: "Cyprus holidays are closed automatically. Add extra days off here.",
    publicHolidays: "Cyprus public holidays",
    extraClosures: "Extra studio closures",
    extraClosuresSub: "Vacations or extra days off. Blocked days will not appear on the public calendar.",
    blockTitle: "Title",
    blockTitlePlaceholder: "e.g. Studio holiday",
    from: "From",
    to: "To",
    blockDates: "Block dates",
    adding: "Adding…",
    noExtraClosures: "No extra closures",
    loadingClosures: "Loading closures",
    loadClosuresError: "Could not load closures",
    missingFields: "Missing fields",
    fillTitleDates: "Fill in title and dates",
    couldNotAddBlock: "Could not add block",
    blocked: "Blocked",
    datesBlocked: "Dates blocked from booking",
    couldNotRemoveBlock: "Could not remove block",
    websiteTitle: "Website content",
    websiteSub: "Edit homepage text, images, and contact details shown to customers",
    saveChanges: "Save changes",
    tabEnglish: "English",
    tabGreek: "Greek (ΕΛ)",
    tabMedia: "Images & contact",
    loadingWebsite: "Loading website content",
    loadWebsiteError: "Could not load website content",
    couldNotSave: "Could not save",
    websiteUpdated: "Website content updated",
    seoTab: "SEO & browser tab ({locale})",
    pageTitle: "Page title",
    metaDescription: "Meta description",
    heroSection: "Homepage hero ({locale})",
    eyebrow: "Eyebrow",
    headline: "Headline",
    description: "Description",
    bookButton: "Book button",
    heroImageAlt: "Hero image alt text",
    aboutSection: "About section ({locale})",
    sectionTitle: "Section title",
    introduction: "Introduction",
    certificationsIntro: "Certifications intro",
    specialization: "Specialization paragraph",
    philosophyTitle: "Philosophy title",
    philosophy1: "Philosophy paragraph 1",
    philosophy2: "Philosophy paragraph 2",
    programIntro: "Program intro",
    closingTitle: "Closing title",
    closingText: "Closing text",
    hoursLine: "Hours line",
    hoursLineHint: "Displayed under hero & footer",
    studioDetails: "Studio details",
    studioDetailsSub: "Shown on the homepage, footer, and Google listing. Phone: +357 95 519786",
    studioName: "Studio name",
    addressEn: "Address / location (English)",
    addressEl: "Address / location (Greek)",
    phone: "Phone",
    email: "Email",
    priceFrom: "Price from (€)",
    heroImage: "Hero image",
    heroImageHint: "Upload a photo of your studio. The current picture is a placeholder (JPEG, PNG, or WebP, max 5MB)",
    reformerImage: "Reformer image",
    reformerImageHint: "Second studio photo — use a real room or reformer shot, not a stock placeholder",
    replaceImage: "Replace image",
    uploading: "Uploading…",
    uploadFailed: "Upload failed",
    imageUpdated: "{title} image updated and saved",
  },
  language: {
    ariaLabel: "Language",
  },
  common: {
    hours: "Tue, Thu, Sat · see booking calendar",
    error: "Error",
    continue: "Continue",
    processing: "Processing…",
    cancel: "Cancel",
    confirm: "Accept",
    dismiss: "Got it",
  },
};

export default en;
