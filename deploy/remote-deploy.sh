...
chmod +x deploy/backup-studio-data.sh deploy/restore-studio-data.sh deploy/restore-from-ops.sh deploy/publish-ops-from-server.sh deploy/ensure-backup-key.sh deploy/ci-deploy-wrapper.sh deploy/write-ops-inventory.sh deploy/bootstrap-from-ops.sh deploy/setup-ops-repo.sh deploy/monitor-studio.sh deploy/prune-disk.sh deploy/setup-disk-hygiene.sh
echo "==> Disk hygiene (journald + logrotate + prune)..."
./deploy/setup-disk-hygiene.sh || true
