...
This file is the operational runbook. It belongs only in the **private** ops
repository. Do not copy host addresses, SSH commands, or this repository name
back into the public `meti-booking` repo.

**This repository:** `panagiod/meti-studio-ops`

The public GitHub repo (`meti-booking`) must not hold customer bookings or the
live `.env`. Encrypted backups are written here every night and after each
deploy. You can restore the live VPS, or rebuild a replacement if this one is
destroyed.
...
GitHub can reach the VPS, but the saved `PRODUCTION_SSH_KEY` secret is not a
full private key (only 513 characters). Backups therefore run **on the server**
and push out. The server needs the same token:
...
That pulls `latest.db.enc` and `secrets/env.enc`, decrypts them, and checks the
database. The live site stays up.

To prove a live swap (brief downtime): **Actions → Backup and Restore** → type
`RESTORE`. That backs up first, then replaces `data.db`, removes WAL files, and
rolls back if `/api/health` fails.

## Restore on the same machine

The live VPS already has `BACKUP_ENCRYPTION_KEY` and `OPS_REPO_TOKEN` in `.env`.
It pulls the encrypted file from this repo itself.
...
2. Add an **unrestricted** SSH public key on that machine (the current deploy
   key is locked to the old wrapper and cannot install a fresh server).
...
6. SSH to the new server, run `./deploy/setup-cicd.sh`, and update
...
The rebuild installs the app first, then restores customers last, so a schema
push cannot wipe the recovered database.

## What never goes in the public repo as plaintext
...
- The production host IP or `ssh root@…` with a live address
- Notes about incomplete deploy keys or other live security debt

## Until the private repo token is added

Nightly backups still fall back to the public `backups` branch as encrypted
files. After `OPS_REPO_TOKEN` works, new backups stay private. Confirm that
fallback is off once this repo is receiving nightly files.
...
The same problem is emailed once, then again after 6 hours if it is still
wrong. A “healthy again” email is sent when it clears.

Logs: `/var/log/meti-booking/monitor.log`

**From GitHub** (Actions → **Uptime**): curls the public site from outside the
VPS. This is the only check that still runs if the machine itself is dead. It
emails only if `RESEND_API_KEY` is also a **GitHub** Actions secret (the copy
in server `.env` is not visible to Actions). Optional: add
`STUDIO_NOTIFICATION_EMAIL` as a GitHub secret too.
